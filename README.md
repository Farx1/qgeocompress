...
**Latest validated result — DOTAv1, 250 held-out val images, 16 718 ground-truth boxes.**
The pretrained checkpoint never trained on this split, so nothing below is contaminated.
Baseline mAP50 0.4136. Deltas are **paired** bootstraps over images.

| Pipeline | Params Δ | mAP50 | Δ vs baseline (95% CI) | CER@0.8 |
| -------- | -------: | ----: | ---------------------- | ------: |
| Shipped plain SVD, r=0.9 | −0.34% | 0.3966 | −0.017 [−0.028, +0.001] | 0.032 |
| Shipped plain SVD, r=0.84 (default) | −2.29% | 0.3776 | −0.036 [−0.058, −0.017] | 0.030 |
| Data-aware budget | −15.83% | 0.4149 | **+0.001 [−0.013, +0.017]** | 0.023 |
| **Data-aware budget** | **−21.34%** | 0.4044 | **−0.009 [−0.027, +0.007]** | 0.021 |
| Data-aware budget | −28.00% | 0.3943 | −0.019 [−0.037, −0.001] | 0.018 |
| **Data-aware budget** | **−34.21%** | 0.3537 | −0.060 [−0.078, −0.042] | 0.013 |

- **−21.3% of parameters with no measurable accuracy cost** — the confidence interval on the
  delta contains zero. The shipped default loses three times more at −2.29%.
- **−34.2% inside the project's gate** (mAP50 drop ≤ 0.10, CER@0.8 delta ≤ 0.02, params ≥ 2%).
- CER@0.8 *improves* monotonically with compression (0.032 → 0.013): the compressed models are
  less overconfident than the baseline, not more.

The same pipeline on the bundled DOTA128 subset gives −16.2% lossless and −32.5% inside the
gate; see [`results/reports/`](results/reports/) for both frontiers. DOTAv1 is the one to
quote — bigger, and the baseline never saw it.
...
| **BatchNorm recalibration as shipped** | Rejected — it was the dominant cost | 20–24 single-image batches at momentum 0.1 leave the pretrained running statistics `0.9**24` = 8% of their weight. Measured on the **uncompressed** baseline, where it should be a no-op: n=24 costs **−0.077 mAP50**, n=4 costs +0.0005, and resetting the statistics to average exactly over 24 images costs −0.22. Roughly 80% of every arm's accuracy loss was this, not compression. Default lowered to 4. |
| **Iterative surrogate calibration** | Rejected | The allocation prices layers by their error measured *in isolation*, which stops being the network's error once 61 layers are factorized together. Measuring each layer's true marginal inside the assembled model and rescaling its weight by the discrepancy moves the frontier by +0.007 / +0.000 / +0.004 / −0.004 mAP50 across four budgets — inside the noise. `--refine` keeps it available. |
| **Output-side (BatchNorm) metric** | Rejected | Nothing reads a conv's output directly; BatchNorm scales it by `γ/σ` first, so the truncation should weight rows by that factor. It helps at −20.7% (0.881 vs 0.866, CIs overlapping) and collapses at −41.0% (0.489 vs 0.699): dividing the second factor by a small `γ/σ` amplifies badly-conditioned channels exactly when rank is scarcest. |
...
- **Two datasets, on purpose** — DOTAv1 val (250 images, 16 718 boxes, uncontaminated) for the headline, and the bundled DOTA128 subset for quick iteration. Absolute mAP50 differs sharply between them (0.41 vs 0.89): DOTA128 tiles are small easy crops, DOTAv1 val is full aerial imagery with hundreds of small objects scored whole-image rather than tiled. Compare deltas within a dataset, never absolute values across them.
