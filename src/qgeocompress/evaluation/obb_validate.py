...
def validate_no_fuse(
    ...
    """Run Ultralytics validation without Conv+BN fusion (required for low-rank layers)."""
    ...
    def _load_without_fuse(self, weight: str | torch.nn.Module) -> None:
        ...
        self.stride = max(int(pytorch_model.stride.max()), 32) if hasattr(pytorch_model, "stride") else 32
        self.names = pytorch_model.module.names if hasattr(pytorch_model, "module") else getattr(pytorch_model, "names", {})
...
def predict_no_fuse(model: YOLO, **kwargs: Any) -> Any:
    """Run Ultralytics predict without Conv+BN fusion (required for structural low-rank)."""
    from ultralytics.nn.backends.pytorch import PyTorchBackend
